...
/**
 * Created by QuyDV on 5/11/17.
 */

public class RecommentItemAdapter extends RecyclerView.Adapter{

    private List<Item> mNumberData;
    private MainActivity mContext;
    private OnItemClickListener mListener;

    public RecommentItemAdapter(List<Item> data, MainActivity context){
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, final int position) {
        ...
        PicassoLoader.getInstance(mContext).with(mContext).load(imgUrl).into(((RecommentItemAdapter.ViewHolder)holder).imgItem);
